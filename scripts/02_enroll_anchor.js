...
async function main () {
  ...
  await invoiceFactroyUpgrade.connect(provider.getSigner(admin.address)).enrollAnchor(anchor.address)
  await invoiceFactroyUpgrade.connect(provider.getSigner(trust.address)).trustVerifyAnchor(anchor.address)
}
